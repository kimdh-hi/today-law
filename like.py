...
def like_star():
    try:
        ...
        if id_receive in like_laws['like_law_id']: # 좋아요가 이미 눌린 법안인 경우
            # 사용자의 좋아요 목록에서 제거
            db.users.update(
                {'user_id':user['user_id']}, # 현재 인증된 사용자로 DB 조회
                {'$pull': # 리스트에서 제거
                     {'like_laws': # like_laws 리스트의 요소중 like_law_id 필드가 id_receive인 요소
                          {'$elemMathch': {'like_law_id': id_receive}}
                      }
                 }
            )
...
def hate_star():
    try:
        ...
        if id_receive in hate_laws['hate_law_id']:

            db.users.update(
                {'user_id':user['user_id']},
                {'$pull':
                     {'hate_laws':
                          {'$elemMathch': {'hate_law_id': id_receive}}
                      }
                 }
            )
